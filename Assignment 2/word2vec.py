...
def naiveSoftmaxLossAndGradient(
    ...
    softmax_out = softmax(np.matmul(outsideVectors, centerWordVec))
    loss = -np.log(softmax_out[outsideWordIdx])
    gradCenterVec = np.matmul(softmax_out, outsideVectors) - outsideVectors[outsideWordIdx]
    gradOutsideVecs = np.outer(softmax_out, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))) \
        - np.sum(np.log(sigmoid(-np.dot(outsideVectors[negSampleWordIndices], centerWordVec))))
    
    # print((1 - sigmoid(-np.dot(outsideVectors[negSampleWordIndices], centerWordVec))).shape)
    # print((1 - sigmoid(-np.dot(outsideVectors[negSampleWordIndices], centerWordVec)))[:, np.newaxis].shape)
    ...
    gradCenterVec = (sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec)) - 1) * outsideVectors[outsideWordIdx] \
        + np.sum((1 - sigmoid(-np.dot(outsideVectors[negSampleWordIndices], centerWordVec)))[:, np.newaxis] * \
    ...
    for idx in negSampleWordIndices:
        if idx not in negSampleCounts:
            negSampleCounts[idx] = 0
        negSampleCounts[idx] += 1
    
    for idx, count in negSampleCounts.items():
        gradOutsideVecs[idx] += count * (1 - sigmoid(-np.dot(outsideVectors[idx], centerWordVec))) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        l, gcv, gov = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += l
